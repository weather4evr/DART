...
use params, only: nsats_rad, nsatmax_rad, sattypes_rad, num_ufo_procs, & 
                     npred, adp_anglebc, emiss_bc, angord, ens_size
use netcdf_mod, only: open_netcdf, close_netcdf, get_netcdf_dims, get_netcdf_var_1d, get_netcdf_var_2d
use mpisetup !, only: stop2, nproc, numproc
...
!   integer(i_kind), allocatable, dimension(:,:) :: model_level_at_peak_of_weightingfunction
    real(r_single), allocatable, dimension(:,:) :: model_level_at_peak_of_weightingfunction ! better to keep as float incase we encounter bad data
       ...
       peloop: do ipe=0,num_ufo_procs-1 ! PEs start at 0, hence the -1
          ...
          if ( num_ufo_procs == 1 ) then
            !obsfile = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_all_'//trim(adjustl(id))//'.nc4'
             obsfile = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_all.nc4'
             geofile = trim(adjustl(obspath))//'/geovals_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_all.nc4'
             ydiagfile = trim(adjustl(obspath))//'/ydiag_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_all.nc4'
          else
             ...
             if (.not.fexist) then
                obsfile = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_'//pe_name//'.nc4'
                geofile = trim(adjustl(obspath))//'/geovals_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_'//pe_name//'.nc4'
                ydiagfile = trim(adjustl(obspath))//'/ydiag_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id))//'_'//pe_name//'.nc4'
             endif
          endif
         ...
         !call get_channels_from_var_names(ncfileid,nchans,channels)
          ...
          ! this opens geofile, closes geofile, allocates/fills pressure2d, and returns nlevs,
          call get_pressure_profiles(geofile,nobs_curr,pressure2d,nlevs)
          ...
          do j = 1,nchans
             ...
             call get_netcdf_var_2d(ncfileid_ydiag,this_variable,1,nobs_curr,model_level_at_peak_of_weightingfunction,ydiagfile) ! Model level at peak of weighting function
            !this_variable = 'weightingfunction_of_atmosphere_layer_'trim(adjustl(ichan))

             do i = 1,nobs_curr
                num_obs_totdiag = num_obs_totdiag + 1 ! total number of obs read...not necessarily total number used
                
                idx = int(model_level_at_peak_of_weightingfunction(1,i)) ! index is top down
                if ( idx > nlevs .or. idx < 0 ) cycle ! If this is true, it's a bad ob
                ...
                if ( pressure(i) < 0.001_r_kind .or. pressure(i) > 1200._r_kind .or. &
                         abs(observation(i)) > 1.e9_r_kind .or. &
                             observation(i) < 100._r_kind ) cycle
...
subroutine get_satobs_data(obspath, datestring, nobs_max, h_x, h_xnobc, x_obs, x_err, &
  ...
  character(len=4)   :: pe_name, ichan
  character(len=100) :: this_variable
  integer(i_kind) :: nob, nsat, ipe, i, j, idx, nn, ii, nlevs, nbad, iret
  ...
  integer(i_kind), allocatable, dimension (:) :: gsiUseFlag, effectiveQC, effectiveQC2, channels
  real(r_single), allocatable, dimension (:) :: initialObsError, finalObsError, all_data
  ...
  logical, allocatable, dimension (:) :: twofiles_all
  ...
  real(r_single), allocatable, dimension(:,:) :: pressure2d, all_data2
! integer(i_kind), allocatable, dimension(:,:) :: model_level_at_peak_of_weightingfunction
  real(r_single), allocatable, dimension(:,:) :: model_level_at_peak_of_weightingfunction ! better to keep as float incase we encounter bad data
  ...
  ! make consistent with screenobs
    ...
    nob = 0
    nbad = 0
    allocate(all_data(numproc))

    ! so far, twofiles_all isn't used yet, but it's kind of fun to keep track of
    allocate(twofiles_all(numproc))
    call mpi_allgather(twofiles, 1, mpi_logical, twofiles_all(:), 1, mpi_logical, mpi_comm_world, iret)
    write(*,*)'num twofiles = ',count(twofiles_all)
    ...
    do nsat=1,nsats_rad
       ...
       peloop: do ipe=0,num_ufo_procs-1 ! PEs start at 0, hence the -1
          ...
          if (twofiles) then
             ...
             if ( num_ufo_procs == 1 ) then
               !obsfile2 = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_all_'//trim(adjustl(id2))//'.nc4'
                obsfile2 = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id2))//'_all.nc4'
             else
                ...
                if (.not.fexist) obsfile2 = trim(adjustl(obspath))//'/obsout_omb_'//trim(adjustl(sattypes_rad(nsat)))//'_'//trim(adjustl(id2))//'_'//pe_name//'.nc4'
             endif
          ...
          allocate( model_level_at_peak_of_weightingfunction(1,nobs_curr))
          allocate( effectiveQC2(nobs_curr))
          allocate(all_data2(nobs_curr,numproc))
         ...
         !call get_channels_from_var_names(ncfileid,nchans,channels)
          ...
          do j = 1,nchans
            ...
            !call get_netcdf_var_1d(ncfileid,this_variable,nobs_curr,Obs_Minus_Forecast_adjusted) ! y-h(x)
             ...
             this_variable = 'brightness_temperature_'//trim(adjustl(ichan))//'@hofx' ! does this include bias correction?
             ...
             if (twofiles) then
                ...
                this_variable = 'brightness_temperature_'//trim(adjustl(ichan))//'@EffectiveQC'
                call get_netcdf_var_1d(ncfileid2,this_variable,nobs_curr,effectiveQC2)
             else
                Obs_Minus_Forecast_unadjusted2 = Obs_Minus_Forecast_unadjusted ! y - h(x)
                effectiveQC2 = effectiveQC
             endif
           ...
           !call get_netcdf_var_1d(ncfileid,'cloud_liquid_water_bias_correction_term@TestReference',nobs_curr,BCPred_Cloud_Liquid_Water)
            ...
            if (index(sat_type(1:4),'ssmi') .gt. 0) then
               ...
               BCPred_Cosine_Latitude_times_Node = 0.0
               BCPred_Sine_Latitude = 0.0
            endif
           ...
           !scanAngle = scanAngle * deg2rad
            ...
            !call get_netcdf_var_1d(ncfileid,'BC_Fixed_Scan_Position',nobs_curr,BC_Fixed_Scan_Position)
             ...
             call mpi_allgather(observation - Obs_Minus_Forecast_unadjusted2, nobs_curr, mpi_real, all_data2(:,:), nobs_curr, mpi_real, mpi_comm_world, iret)
             do i = 1,nobs_curr
               ...
               !if ( .not. twofiles ) then
                 !if ( BCPred_Emissivity(i) > 0 ) write(*,*)'qc,lat,lon,emiss = ',effectiveQC(i), latitude(i),longitude(i),BCPred_Emissivity(i)
               !endif
                if (effectiveQC(i) > 0 ) cycle
                if ( pressure(i) < 0.001_r_kind .or. pressure(i) > 1200._r_kind .or. &
                         abs(observation(i)) > 1.e9_r_kind .or. &
                ...
                ! check to make sure the radiance forward operator didn't fail for any ob
               !call mpi_allgather(observation(i) - Obs_Minus_Forecast_unadjusted2(i), 1, mpi_real, all_data(:), 1, mpi_real, mpi_comm_world, iret)
                all_data(:) = all_data2(i,:)
                if ( any(all_data > 350.0) .or. any(all_data < 100.0) ) then
                   if ( nproc == 0 ) write(*,*)'bad H(x), lat/lon/sat/chan = ',latitude(i),longitude(i),sat_type,this_channel
                   write(*,*)'H(x)/qc = ',observation(i) - Obs_Minus_Forecast_unadjusted2(i), effectiveQC2(i)
                   nbad = nbad + 1
                   initialObsError(i) = 100. ! we can't "cycle", but we can set the obs error to something outrageous to effectively make the ob do nothing
                   finalObsError(i) = 100.
                endif
               ...
               !h_x(nob) = x_obs(nob) - Obs_Minus_Forecast_unadjusted(i) ! assume H(x) includes B
                h_x(nob) = x_obs(nob) - Obs_Minus_Forecast_unadjusted(i) + obsBias(i) ! assume H(x) does NOT include B

                ! un-bias corrected Hx for the member
               !h_xnobc(nob) = (x_obs(nob) - obsBias(i)) - Obs_Minus_Forecast_unadjusted2(i) ! assume H(x) includes B
                h_xnobc(nob) = x_obs(nob) - Obs_Minus_Forecast_unadjusted2(i) ! assume H(x) does NOT include B
...
! from GSI's radinfo: radiance bias correction terms are as follows:
...
!   else
       if (nproc == 0 ) then
          print *,'number of radiance obs with failed forward operators ',nbad
!         print *,'number of radiance obs with good forward operators ',nob
!         print *,'expected number of radiance obs ',nobs_max
       endif
!   endif
...
subroutine get_pressure_profiles(fname,nobs_curr,pressure2d,nlevs)
   character(len=*), intent(in) :: fname
   integer(i_kind), intent(in)  :: nobs_curr
   real(r_single), allocatable, dimension(:,:), intent(inout) :: pressure2d
   integer(i_kind), intent(inout)  :: nlevs

   integer(i_kind) :: ncid, nobs2

   call open_netcdf(fname,ncid) ! open file
   call get_netcdf_dims(ncid,'air_pressure_nval',nlevs)

   ! sanity check
   ! if the test fails, there are several variables that will be unallocated when program quits
   call get_netcdf_dims(ncid,'nlocs',nobs2)
   if ( nobs_curr /= nobs2 ) then
      call close_netcdf(fname,ncid) ! close ensemble mean file
      call stop2(83)
   endif
