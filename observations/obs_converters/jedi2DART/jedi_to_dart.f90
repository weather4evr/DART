program jedi_to_dart

use kinds
use mpisetup
use params, only : datestring, datapath, convert_conv, lie_about_ob_times, recenter_about_mean_prior, &
                   convert_sat, sattypes_rad, sattypes_oz, ens_size, obs_seq_out_filename, &
                   nsats_rad,nsats_oz,nsatmax_rad,nsatmax_oz, num_obs_platforms_max, num_ufo_procs, &
                   obsprd_prior, ensmean_obnobc, ensmean_ob, ob, oberrvar, oberrvar_orig, output_bias_preds, &
                   ...
                   exclude_these_obs_types, output_option, anal_ob_chunk, obs_platforms, num_obs_platforms, &
                   npred, adp_anglebc, angord, emiss_bc, process_sondes_Tv
...
! namelist variables are declared and initialized in params
namelist /jedi_to_dart_nml/ ens_size, &
   convert_conv, convert_sat, datestring, datapath, sattypes_rad, sattypes_oz, &
   write_FO_for_these_obs_types, write_prior_copies, &
   exclude_these_obs_types, output_option, obs_seq_out_filename, &
   lie_about_ob_times, recenter_about_mean_prior, num_ufo_procs, output_bias_preds, obs_platforms, &
   npred,adp_anglebc,angord,emiss_bc, process_sondes_Tv
...
! Print out some info
call initialize_utilities('jedi2dart')

! Read namelist on all PEs
call find_namelist_in_file("input.nml", "jedi_to_dart_nml", unitnml)
read(unitnml, nml = jedi_to_dart_nml, iostat = io)
call check_namelist_read(unitnml, io, "jedi_to_dart_nml")
...
! Do some error checking... stop2 subroutine from mpisetup
...
! find number of conventional observation platforms
num_obs_platforms = 0
do i=1,num_obs_platforms_max
   if ( obs_platforms(i) == ' ') cycle
   num_obs_platforms = num_obs_platforms + 1
   if(nproc == 0)write(6,*) 'Processing platform ',trim(adjustl(obs_platforms(i)))
enddo
if(nproc == 0)write(6,*) 'number of conventional observation platforms', num_obs_platforms
