...
use         types_mod, only : obstypelength ! CSS needs to be added for DART Manhattan
use      obs_kind_mod
...
subroutine dart_obs_seq (datestring,                              &
   ...
   real(r_kind)            :: these_bias_preds(npred+2) ! npred from params
...
! set observation time type
   ...
   obsloop: do i = nobs_start, nobs_end
      ...
      if ( is_conv ) then
         ...
         call radiance_to_dart_obs_kind(obtype(i),stattype(i),obskind,which_vert,obs_kind_gen)
      end if
      ...
      if ( obs_kind_gen == QTY_GPSRO ) then 
         vloc = obpress(i) !GPS obserations have height in meters, which is correct
      else
         vloc = obpress(i)*100.0 !Convert to Pa if not GPSRO
      endif
      lon  = obloclon(i)
      lat  = obloclat(i)
...
!        ens_copy(2:ens_size+1) = anal_ob_chunk(1:ens_size,i)   ! CSS scatterv
      ...
      ! DART wants lons bewteen 0 and 360
      do while ( lon < 0.0 ) 
         lon = lon + 360.0
      enddo
      ...
      call set_obs_def_type_of_obs(obs_def, obskind) ! Pre-manhattan, needed to call "set_obs_def_kind(obs_def, obskind)" from obs_def_mod (and add in "use-list")
      ...
      if ( is_sat .and. convert_sat ) then
         if ( output_bias_preds ) then ! from namelist
...
!------------------------------------------------------------------------------
   ...
   if(obtype(1:3) == 'gps' .or. trim(adjustl(obtype)) == 'refractivity' .or. &
                                trim(adjustl(obtype)) == 'bending_angle') then
     obs_kind_gen = QTY_GPSRO ! CSS bug fix
...
!  if( trim(adjustl(obtype)) == 'virtual_temperature') then
!    obs_kind_gen = QTY_TEMPERATURE
!    if(obstype == 120 .or. obstype == 132) obs_kind = RADIOSONDE_VIRTUAL_TEMP
!  endif

   if(obtype(1:3) == '  t' .or. trim(adjustl(obtype)) == 'air_temperature') then
     obs_kind_gen = QTY_TEMPERATURE
   ...
   if(obtype(1:3) == '  q' .or. trim(adjustl(obtype)) == 'specific_humidity' ) then
     ...
     if(obstype == 133 .or. obstype == 134) obs_kind = ACARS_SPECIFIC_HUMIDITY  ! Really should be AMDAR per HRRRE changes, but that requires some obs_def/preprocess changes.
     ...
     if(obstype == 188 )                    obs_kind = MESONET_SPECIFIC_HUMIDITY! CSS 
   endif

   if(obtype(1:3) == ' ps' .or. trim(adjustl(obtype)) == 'surface_pressure') then
   ...
   if(obtype(1:3) == '  u' .or. trim(adjustl(obtype)) == 'eastward_wind') then
   ...
   if(obtype(1:3) == '  v' .or. trim(adjustl(obtype)) == 'northward_wind') then
   ...
   which_vert = VERTISPRESSURE ! assume pressure vertical coordinate
   ! in MPAS-JEDI, GNSS pressures not available, but height is.
   if(obtype(1:3) == 'gps' .or. trim(adjustl(obtype)) == 'refractivity' .or. &
                                trim(adjustl(obtype)) == 'bending_angle') then
...
subroutine radiance_to_dart_obs_kind(obtype, channel, obs_kind, which_vert, obs_kind_gen)
   ...
   obs_kind =  get_index_for_type_of_obs(this_string)  ! get_index_for_type_of_obs from obs_kind_mod.  Pre-manhattan, this subroutine was "get_obs_kind_index" from obs_kind_mod
   obs_kind_gen = QTY_BRIGHTNESS_TEMPERATURE
...
function write_this_ob_type_external_FO(ob_type)
   ...
   integer                            :: i
   logical                       :: is_all
   character(len = obstypelength)  :: ob_type_string  ! obstypelength from types_mod ... Pre-manhattan this was a constant from obs_kind_mod (something like "paramlength")
   ...
   ob_type_string = get_name_for_type_of_obs(ob_type)   ! from obs_kind_mod. Pre-manhattan, this subroutine was "get_obs_kind_name" from obs_kind_mod
