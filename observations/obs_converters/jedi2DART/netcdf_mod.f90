...
subroutine get_netcdf_var_2d_real(fileid,variable,dim1,dim2,output,fname)
   ...
   if ( ierr /= 0 ) then
      if ( present(fname)) then
         write(0,fmt='(a300)') 'Error reading data for '//trim(adjustl(variable))//' from '//fname
      else
         write(0,*) 'Error reading data for '//trim(adjustl(variable))
      endif
      write(0,*) 'ierr = ',ierr
...
subroutine get_netcdf_var_2d_integer(fileid,variable,dim1,dim2,output,fname)
   ...
   ierr = 0
   ncstatus = nf90_inq_varid(fileid,trim(adjustl(variable)),ncvarid) ; ierr = ierr + ncstatus
!  write(*,*)'ncstatus1 = ',ncstatus
   ncstatus = nf90_get_var(fileid,ncvarid,output)                    ; ierr = ierr + ncstatus
!  write(*,*)'ncstatus2 = ',ncstatus
