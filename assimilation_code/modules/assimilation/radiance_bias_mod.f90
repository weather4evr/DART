...
! public variables
public :: numiter, lupd_satbiasc, nobs_sat, use_HofX_ensmean
...
logical            :: use_HofX_ensmean = .false.
...
namelist / radiance_bias_nml /  numiter, lupd_satbiasc, bias_correction_source, biasvar, use_HofX_ensmean, &
   ...
   if ( lupd_satbiasc ) then
      ...
      if ( nobs_sat > 0 ) then
         ...
         if ( use_HofX_ensmean ) then
            write(msgstring, '(A)') 'Using forward operator applied to ensemble mean prior for bias correction update, like in the EnSRF'
            call error_handler(E_MSG,'radiance_bias_init:', msgstring)
         endif
...
subroutine apply_biascorr(obs_seq,obs_ens_handle,ens_size,prior_H_x_ensmean)
   type(obs_sequence_type),  intent(in)    :: obs_seq
   type(ensemble_type),      intent(inout) :: obs_ens_handle
   integer,                  intent(in)    :: ens_size
   real(r8),                 intent(inout) :: prior_H_x_ensmean(obs_ens_handle%my_num_vars)
   ! Update the radiance priors based on the latest bias correction coefficients that have 
   !  been updated in update_biascorr
   if ( trim(adjustl(bias_correction_source)) == 'gsi') then
      call apply_biascorr_gsi(obs_seq,obs_ens_handle,ens_size,prior_H_x_ensmean)
   endif
   call error_handler(E_MSG, 'apply_biascorr:', 'Done with apply_biascorr') 
end subroutine apply_biascorr

subroutine update_biascorr(niter,obs_seq, obs_ens_handle, keys, obs_val_index, OBS_MEAN_COPY, ens_size, OBS_GLOBAL_QC_COPY, prior_H_x_ensmean )
   integer(i8), intent(in)               :: niter
   type(obs_sequence_type),  intent(in)  :: obs_seq
   type(ensemble_type), intent(inout)    :: obs_ens_handle
   integer, intent(in)                   :: keys(:)
   integer, intent(in)                   :: obs_val_index, OBS_MEAN_COPY, ens_size, OBS_GLOBAL_QC_COPY
   real(r8), intent(in)                  :: prior_H_x_ensmean(obs_ens_handle%my_num_vars)
   ...
   call get_global_radiance_info(obs_seq, obs_ens_handle, keys, obs_val_index, OBS_MEAN_COPY, ens_size, OBS_GLOBAL_QC_COPY, prior_H_x_ensmean )
...
subroutine get_global_radiance_info(obs_seq, obs_ens_handle, keys, obs_val_index, OBS_MEAN_COPY, ens_size, OBS_GLOBAL_QC_COPY, prior_H_x_ensmean )
   ...
   if ( use_HofX_ensmean ) then
      obs_ens_handle%copies(OBS_MEAN_COPY, :) = prior_H_x_ensmean(:) ! Based on the actual ensemble mean prior (H(x_bar))
   else
...
subroutine apply_biascorr_gsi(obs_seq,obs_ens_handle,ens_size,prior_H_x_ensmean)
   type(obs_sequence_type),  intent(in)    :: obs_seq
   type(ensemble_type),      intent(inout) :: obs_ens_handle
   integer,                  intent(in)    :: ens_size
   real(r8),                 intent(inout) :: prior_H_x_ensmean(obs_ens_handle%my_num_vars)
         ...
         if (base_obs_kind == QTY_BRIGHTNESS_TEMPERATURE) then
            ...
            do j = 1,jpch_rad
               ...
               if ( trim(adjustl(full_ob_name)) == trim(adjustl(base_obs_name)) ) then ! we found a match for this satellite/sensor/channel.
                  ...
                  ! Note that prior_H_x_ensmean has all obs on this PE, not just radiances. Fortunately, we saved the indices we need to update (my_radiance_ob_indices)
                  ...
                  if (.not. adp_anglebc) then
                     ...
                     if ( use_HofX_ensmean ) prior_H_x_ensmean(ii) = prior_H_x_ensmean(ii) - bias_amount + these_biaspreds(npred+1) 
                  else
                     obs_ens_handle%copies(1:ens_size,ii) = obs_ens_handle%copies(1:ens_size,ii) - bias_amount
                     if ( use_HofX_ensmean ) prior_H_x_ensmean(ii) = prior_H_x_ensmean(ii) - bias_amount
                  endif
                  ...
                  do np=1,npred
                     ...
                     if ( use_HofX_ensmean ) prior_H_x_ensmean(ii) = prior_H_x_ensmean(ii) + these_biaspreds(np)*(predx(np,j)+deltapredx(np,j))
                  enddo
